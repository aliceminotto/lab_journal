...
* arduino meeting and workshop

* reading about different random number generators (to figure out what's the deal with my code). Notes on this:
  * of course the number returned are just _pseudo_-random, cause thay are generated by an algorithm. For obvious reason you an't use these generators for cryptography or security reasons.
  * a lot of standard library for random generators were proved to be very faulted (like {Python before v 2.3, Perl, Matlab and Java, C++). So always use something better if it exists. At the moment Python <code>random()</code> is considered a very good random number generator, and the same for <code>numpy.random()</code>. They both use the **Marsenne Twister** as algorithm (considered the best one nowadays?).
  * usually u would want to seed the generator in particoular occasions only (in Python in particoular the loop of random number is veeeeeeeeeeeery long so they are NOT going to be repeated in any useful application i could think of). (Anyway Python random() and numpy.random() are both seeded with the time, not 0, so no problems ghere). You may want to seed the generaotr for test purposes (to see exactly what your changes in the code are causing in the output), OR, if you are launching a lot of the same process all togheter for stathistical reasons: most of them (or anyway some of them), will be starting at the same time and be seeded with the same number, so the stathistic will be horrible. In this cas eyou need to seed the generaotr. For the launchng of several processes our code was already safe cause the seed was being changed between the different processes depending on the number of the process.

* todo: look at <code>functool.partial()</code>
